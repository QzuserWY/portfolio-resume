...
export const useEmails = () => {
  const isEmailSendingEnabled = () => {
    return import.meta.env.PROD || import.meta.env.VITE_ENABLE_EMAILS === "true"
  }
  ...
  const init = (publicKey, serviceId, templateId) => {
    ...
    if (!publicKey || !serviceId || !templateId)
      throw new Error(
        "Error initializing emails.js! Make sure you informed all parameters correctly.",
      )
  ...
  const sendContact = async (fromName, fromEmail, customSubject, message) => {
    if (!isEmailSendingEnabled()) return true
    ...
    try {
      await emailjs.send(_params.serviceId, _params.templateId, requestParams)
      return true
    } catch (error) {
      return false
    }
